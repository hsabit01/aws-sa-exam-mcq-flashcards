...
### What is it?
Vertical scalability means making one server bigger.

This is also called scaling up.

In AWS, this usually means changing to a larger EC2 instance type with more CPU, RAM, or network performance.

### How it works?
You increase the size of one machine instead of adding more machines.

For example, you can move from a small EC2 instance to a larger one.

This gives the application more power, but it still depends on one server unless you add other high availability features.
...
flowchart LR
    U([👤 User Traffic]):::user --> A[🌐 App]:::app
    A --> S1[🖥️ Small EC2]:::data
    S1 --> S2[🚀 Bigger EC2]:::core
    S2 --> V[📈 More CPU / RAM]:::dash
...
| Vertical Scalability | Making one server bigger | Increase instance size to get more CPU, RAM, or power | Databases or legacy apps that do not scale out easily | Scale up, bigger instance, more memory, single server |
