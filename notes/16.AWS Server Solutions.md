## Lambda

### What is it?
AWS Lambda is a serverless compute service.

You run code without managing servers.
AWS handles scaling, patching, and availability.

You usually pay for requests and execution time.
It is a strong exam answer when the question wants event-driven, short-running, managed compute.

### How it works?
You upload code or a container image.

An event triggers the function, such as API Gateway, S3, EventBridge, or DynamoDB Streams.

Lambda runs your code in an execution environment, returns the result, and scales automatically by creating more environments when needed.

### Use Case
A user uploads an image to S3.

S3 triggers Lambda, and Lambda resizes the image and stores the new copy.

### Exam Tip
Pick Lambda when the exam says:
serverless, event-driven, automatic scaling, no server management, pay per use.

Common trap:
do not choose Lambda for long-running jobs, full control of the OS, or workloads that need to run continuously.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Event Source]):::user --> A[🌐 Trigger]:::app
    A --> S[(🚀 AWS Lambda)]:::core
    S --> D[(📝 Process Data)]:::data
    S --> V[📈 Response / Action]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Lambda Concurrency Error

### What is it?
A Lambda concurrency error usually means Lambda hit a concurrency limit.

Concurrency means how many function invocations are running at the same time.

When the limit is reached, extra requests can be throttled.

### How it works?
Each running invocation uses one concurrency unit.

If traffic rises and Lambda needs more concurrent executions than are available, Lambda cannot start more right away.

This can happen because of the account concurrency quota or a function's reserved concurrency setting.

### Use Case
An API suddenly gets a traffic spike.

Your Lambda function gets many requests at once, hits the concurrency limit, and clients start seeing throttling errors.

### Exam Tip
Watch for words like:
throttled, TooManyRequests, traffic spike, concurrent executions, quota reached.

Strong answers are:
increase concurrency quota, use reserved concurrency for critical functions, reduce function duration, or buffer traffic with SQS.

Common trap:
Provisioned Concurrency helps with startup latency.
It does not mean infinite concurrency.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Many Requests]):::user --> A[🌐 Lambda Trigger]:::app
    A --> S[(🚀 Lambda Concurrency Pool)]:::core
    S --> V[📈 Runs Requests]:::dash
    S --> D[(📝 Throttled Requests)]:::data

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Lambda Cold Starts & Provisioned Concurrency

### What is it?
A cold start is the extra startup time when Lambda creates a new execution environment.

Provisioned Concurrency keeps execution environments already initialized, so requests start faster.

This is mainly about performance and latency.

### How it works?
Without Provisioned Concurrency, Lambda creates environments on demand.

That setup time can include loading the runtime, your code, libraries, and initialization logic.

With Provisioned Concurrency, Lambda prepares a set number of environments ahead of time.

### Use Case
A customer-facing checkout API must respond very quickly every morning when traffic starts.

Provisioned Concurrency is a good fit because low latency matters.

### Exam Tip
Pick Provisioned Concurrency when the exam says:
latency-sensitive, predictable traffic, low startup delay, consistent response times.

Common trap:
Provisioned Concurrency costs extra.
Do not choose it just because the app uses Lambda.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 User Request]):::user --> A[🌐 API / Event]:::app
    A --> S[(🚀 Pre-Warmed Lambda)]:::core
    S --> V[📈 Fast Response]:::dash
    D[(📝 Init Code Ready)]:::data --> S

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Reserved and Provisioned Concurrency

### What is it?
Reserved Concurrency sets aside a fixed number of concurrent executions for one function.

It also acts as a cap for that function.

Provisioned Concurrency pre-initializes execution environments to reduce cold starts.

### How it works?
Reserved Concurrency protects a function from being starved by other functions.

Provisioned Concurrency prepares warm environments on a version or alias, so invocations start faster.

You can use both together:
reserved helps capacity control, provisioned helps startup speed.

### Use Case
A payment Lambda must always have capacity available.

You assign Reserved Concurrency so other functions cannot consume all account concurrency.
You add Provisioned Concurrency if the payment API also needs low latency.

### Exam Tip
Reserved Concurrency = capacity guarantee and limit.

Provisioned Concurrency = warm environments for low latency.

Common trap:
they are not the same thing.
Reserved is about capacity control.
Provisioned is about startup performance.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Requests]):::user --> A[🌐 Lambda Function]:::app
    A --> S[(🚀 Reserved Capacity)]:::core
    S --> D[(📝 Provisioned Warm Envs)]:::data
    D --> V[📈 Fast + Protected Capacity]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Lambda SnapStart

### What is it?
Lambda SnapStart reduces cold start time by restoring a function from a saved initialized snapshot.

It is designed for supported runtimes and published versions.

This is another performance feature, like Provisioned Concurrency, but it works differently.

### How it works?
Lambda runs your initialization code when you publish a function version.

It then saves a snapshot of that initialized environment.

When new execution environments are needed, Lambda restores from that snapshot instead of doing full initialization again.

### Use Case
A Java-based Lambda API has slow cold starts because of framework startup.

SnapStart is a strong answer when you want faster startup without keeping environments continuously warm.

### Exam Tip
Pick SnapStart when the exam says:
slow cold starts, Java or supported runtime, published version, reduce initialization delay.

Common traps:
you cannot use SnapStart and Provisioned Concurrency on the same function version.
Also watch for uniqueness issues, because data created during initialization can be reused after restore.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Publish Version]):::user --> A[🌐 Init Function]:::app
    A --> D[(📝 Snapshot Saved)]:::data
    D --> S[(🚀 Restore Fast)]:::core
    S --> V[📈 Lower Cold Start]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Lambda@Edge & CloudFront Function

### What is it?
Both services run code at CloudFront edge locations.

CloudFront Functions is for lightweight, very fast request and response changes.

Lambda@Edge is for more complex edge logic.

### How it works?
CloudFront Functions runs simple JavaScript on viewer request or viewer response.

Lambda@Edge runs Lambda code closer to users and supports more complex processing, including viewer and origin events.

Use CloudFront Functions for simple header, URL, cookie, or redirect logic.
Use Lambda@Edge for richer processing.

### Use Case
You want to normalize URLs and add security headers at the edge.

Choose CloudFront Functions for that lightweight task.

If you need logic that changes origin requests or does more advanced processing, Lambda@Edge is the better fit.

### Exam Tip
CloudFront Functions clues:
ultra-low latency, lightweight, header rewrite, cache key normalization, simple redirect.

Lambda@Edge clues:
more complex logic, origin request/response handling, full Lambda-style edge compute.

Common trap:
do not choose Lambda@Edge for simple viewer request rewrites when CloudFront Functions is enough and cheaper.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Viewer]):::user --> A[🌐 CloudFront]:::app
    A --> S[(🚀 CloudFront Function)]:::core
    A --> D[(📝 Lambda@Edge)]:::data
    S --> V[📈 Simple Rewrite / Redirect]:::dash
    D --> V

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3,4 stroke:#EAEAEA,stroke-width:3px;
```
## Amazon DynamoDB

### What is it?
Amazon DynamoDB is a fully managed serverless NoSQL database.

It gives single-digit millisecond performance at scale.

It is a common exam answer when the question wants massive scale, low latency, and no database server management.

### How it works?
You store data in tables.

Each table has items and attributes, and every item is identified by a primary key.

DynamoDB automatically handles partitioning, scaling, and high availability.

### Use Case
A gaming app stores player profiles and session data with very high read and write traffic.

DynamoDB is a strong choice because it scales well and responds quickly.

### Exam Tip
Pick DynamoDB when the exam says:
NoSQL, key-value, document, massive scale, very low latency, serverless database.

Common trap:
do not choose DynamoDB when the question clearly needs joins, complex relational queries, or traditional SQL features.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 App User]):::user --> A[🌐 Application]:::app
    A --> S[(🚀 DynamoDB Table)]:::core
    D[(📝 Items / Attributes)]:::data --> S
    S --> V[📈 Fast Reads & Writes]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Amazon DynamoDB Accelerator (DAX)

### What is it?
DAX is a fully managed, in-memory cache for DynamoDB.

It is built specifically to make DynamoDB reads much faster.

It is best for read-heavy workloads that need microsecond latency.

### How it works?
Your application uses the DAX client.

For supported read requests, DAX tries to serve data from memory first.

If the item is not cached, DAX gets it from DynamoDB and stores it for future reads.

### Use Case
A flash-sale app reads the same hot product data many times per second.

DAX helps reduce read latency and offloads repeated reads from DynamoDB.

### Exam Tip
Pick DAX when the exam says:
DynamoDB, read-heavy, hot keys, microsecond latency, minimal cache management.

Common trap:
DAX is mainly for DynamoDB read acceleration.
It is not a general-purpose cache for every application.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 App]):::user --> A[🌐 DAX Client]:::app
    A --> S[(🚀 DAX Cache)]:::core
    S --> D[(📝 DynamoDB)]:::data
    S --> V[📈 Microsecond Reads]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## DynamoDB Accelerator (DAX) vs. ElastiCache

### What is it?
Both can reduce read latency, but they are not the same.

DAX is purpose-built for DynamoDB.

ElastiCache is a general in-memory cache service for many use cases.

### How it works?
DAX sits in front of DynamoDB and works with DynamoDB API patterns.

ElastiCache gives you Redis or Memcached, and your application manages how data is cached and invalidated.

DAX is simpler when the problem is specifically DynamoDB read acceleration.

### Use Case
You have a DynamoDB-backed shopping app with hot read traffic.

Choose DAX if you want the easiest managed cache for DynamoDB.

Choose ElastiCache if you need a broader cache for sessions, leaderboards, custom caching patterns, or data from many sources.

### Exam Tip
Choose DAX when the exam says:
speed up DynamoDB reads, microsecond latency, minimal app changes, fully managed DynamoDB cache.

Choose ElastiCache when the exam says:
Redis, Memcached, session store, shared cache, pub/sub, sorted sets, or cache data from more than DynamoDB.

Common trap:
ElastiCache is more flexible.
DAX is more targeted and simpler for DynamoDB read caching.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 App]):::user --> A[🌐 Choice]:::app
    A --> S[(🚀 DAX for DynamoDB)]:::core
    A --> D[(📝 ElastiCache for General Cache)]:::data
    S --> V[📈 Fast DynamoDB Reads]:::dash
    D --> V

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3,4 stroke:#EAEAEA,stroke-width:3px;
```
## DynamoDB – Stream Processing

### What is it?
DynamoDB Streams captures item-level changes in a DynamoDB table.

It is used for change data capture in near real time.

This is a common exam answer when a workflow must react to table inserts, updates, or deletes.

### How it works?
You enable a stream on a DynamoDB table.

When an item changes, DynamoDB writes a stream record.

A consumer such as Lambda reads those records and performs downstream actions.

### Use Case
When a new order is written to a table, a Lambda function reads the stream and sends a confirmation email.

This creates an event-driven design without changing the main app flow.

### Exam Tip
Pick DynamoDB Streams when the exam says:
react to item changes, trigger Lambda on table updates, CDC, event-driven processing.

Common trap:
Streams is not the main database.
It is the change log for the table.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 App Write]):::user --> A[🌐 DynamoDB Table]:::app
    A --> S[(🚀 DynamoDB Stream)]:::core
    S --> D[(📝 Lambda Consumer)]:::data
    D --> V[📈 Email / Update / Audit]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## DynamoDB – Stream Processing vs Kinesis Data Streams (Newer)

### What is it?
Both can capture DynamoDB item changes in near real time.

DynamoDB Streams is the simpler built-in option.

Kinesis Data Streams for DynamoDB is better when you need more streaming power.

### How it works?
DynamoDB Streams keeps item change records for up to 24 hours and is commonly consumed by Lambda.

Kinesis Data Streams for DynamoDB sends table changes into a Kinesis data stream.

That gives you longer retention, more replay options, and better support for multiple independent consumers.

### Use Case
A simple app needs to trigger Lambda when orders change.
Use DynamoDB Streams.

A data platform needs multiple consumers, longer retention, and replay for analytics.
Use Kinesis Data Streams for DynamoDB.

### Exam Tip
Pick DynamoDB Streams when the exam says:
simple CDC, Lambda trigger, low operational overhead.

Pick Kinesis Data Streams for DynamoDB when the exam says:
multiple consumers, replay, longer retention, high-volume streaming, downstream analytics.

Common trap:
If the question needs more than 24-hour retention or multiple consumer applications, Kinesis is usually the better answer.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 DynamoDB Changes]):::user --> A[🌐 CDC Choice]:::app
    A --> S[(🚀 DynamoDB Streams)]:::core
    A --> D[(📝 Kinesis Data Streams)]:::data
    S --> V[📈 Simple Lambda Processing]:::dash
    D --> V

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3,4 stroke:#EAEAEA,stroke-width:3px;
```
## AWS API Gateway

### What is it?
Amazon API Gateway is a fully managed service for creating, publishing, securing, and monitoring APIs.

It supports REST APIs, HTTP APIs, and WebSocket APIs.

It is often the front door for serverless and microservices architectures.

### How it works?
Clients send requests to API Gateway.

API Gateway routes the request to a backend such as Lambda, an HTTP endpoint, or another AWS service.

It can also handle authorization, throttling, request validation, and monitoring.

### Use Case
A mobile app needs a secure HTTPS endpoint to invoke backend Lambda functions.

API Gateway is a strong choice because it gives managed API features without building your own API layer.

### Exam Tip
Pick API Gateway when the exam says:
create APIs, serverless backend, managed API front door, Lambda integration, throttling, authorization.

Common trap:
API Gateway is not the compute service.
It is the managed API entry point in front of your backend.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Client App]):::user --> A[🌐 API Gateway]:::app
    A --> S[(🚀 Lambda / HTTP / AWS Service)]:::core
    D[(📝 Auth / Validation / Throttle)]:::data --> A
    S --> V[📈 API Response]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## AWS API Gateway - Endpoint Types

### What is it?
API Gateway endpoint type means how the API is exposed.

For SAA exam questions, think mainly about REST API endpoint types:
Edge-Optimized, Regional, and Private.

### How it works?
Edge-Optimized uses a CloudFront distribution managed by API Gateway.
It is good for globally distributed clients.

Regional is for clients in the same Region or when you want to control your own CloudFront setup.

Private makes the API accessible only from a VPC using an interface VPC endpoint.

### Use Case
A public global API for users around the world:
Edge-Optimized.

A public API mainly for one Region or behind your own CloudFront:
Regional.

An internal API only for workloads inside a VPC:
Private.

### Exam Tip
Edge-Optimized = global public clients.

Regional = same-Region public clients or custom front-end setup.

Private = internal access from VPC only.

Common trap:
Private API does not mean "public API with auth."
It means network-level private access through VPC endpoints.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Clients]):::user --> A[🌐 API Gateway]:::app
    A --> S[(🚀 Edge-Optimized)]:::core
    A --> D[(📝 Regional / Private)]:::data
    S --> V[📈 Global Access]:::dash
    D --> V

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3,4 stroke:#EAEAEA,stroke-width:3px;
```
## AWS API Gateway - Security

### What is it?
API Gateway has multiple security layers.

You can control who can call the API and protect it from unwanted traffic.

### How it works?
You can secure an API with IAM authorization, Amazon Cognito user pools, or Lambda authorizers.

You can also use resource policies, WAF, throttling, and mutual TLS for stronger protection in the right design.

Different security tools solve different problems:
identity, network access, and traffic filtering.

### Use Case
A public mobile API needs user sign-in and token-based access.

Use Cognito user pools with API Gateway authorizers.

An internal AWS service calling an API can use IAM authorization.

### Exam Tip
IAM auth = AWS users, roles, and SigV4-signed requests.

Cognito user pools = user sign-in and JWT tokens for app users.

Lambda authorizer = custom auth logic.

Resource policy = control who can call the API based on account, IP, or VPC endpoint.

Common trap:
API keys and usage plans are for metering and throttling.
They are not strong authentication by themselves.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Client]):::user --> A[🌐 API Gateway]:::app
    D[(📝 IAM / Cognito / Lambda Authorizer)]:::data --> A
    A --> S[(🚀 Backend Service)]:::core
    A --> V[📈 Allowed or Denied]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## AWS Step Functions

### What is it?
AWS Step Functions is a workflow orchestration service.

It lets you coordinate multiple steps, services, and error-handling rules in a state machine.

It is a very strong exam answer when the problem is about orchestration, retries, branching, and workflow visibility.

### How it works?
You define a workflow made of states such as Task, Choice, Wait, Parallel, or Map.

Each step can call Lambda or many AWS services directly.

Step Functions manages retries, catches errors, passes data between steps, and tracks execution progress.

### Use Case
An order workflow needs payment, inventory check, shipping, and failure handling.

Step Functions is ideal because the process has multiple steps and must handle success and failure clearly.

### Exam Tip
Pick Step Functions when the exam says:
workflow, orchestration, retries, branching logic, visual state machine, human approval, long-running business process.

Also know:
Standard workflows are better for durable, auditable, long-running flows.
Express workflows are better for very high-volume event processing.

Common trap:
Step Functions is not a message queue and not an event bus.
It orchestrates steps.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 Order Event]):::user --> A[🌐 Step Functions]:::app
    A --> S[(🚀 Payment Step)]:::core
    S --> D[(📝 Inventory / Shipping / Retry)]:::data
    D --> V[📈 Success or Failure Path]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Amazon Cognito

### What is it?
Amazon Cognito is AWS's managed service for user identity in apps.

For the exam, remember the two big parts:
User Pools and Identity Pools.

User Pools handle user sign-up and sign-in.
Identity Pools provide temporary AWS credentials.

### How it works?
Users authenticate with a User Pool or another identity provider.

If the app needs access to AWS services like S3, the user can then get temporary AWS credentials through an Identity Pool.

So the common exam pattern is:
User Pool for authentication, Identity Pool for AWS access.

### Use Case
A mobile app lets users sign in and upload photos directly to S3.

Use a User Pool for login.
Use an Identity Pool to give temporary AWS credentials with limited S3 permissions.

### Exam Tip
User Pool = authentication and JWT tokens.

Identity Pool = temporary AWS credentials for access to AWS resources.

Common trap:
a User Pool alone does not give direct AWS resource access.
If the question needs access to S3, DynamoDB, or other AWS services from the app, think Identity Pool.

### Visual Mermaid
```mermaid
%%{init: {'theme':'base','themeVariables': {
  'background':'#0B0F19',
  'primaryTextColor':'#111111',
  'secondaryTextColor':'#111111',
  'tertiaryTextColor':'#111111',
  'lineColor':'#EAEAEA',
  'fontSize':'20px'
}}}%%
flowchart LR
    U([👤 App User]):::user --> A[🌐 Cognito User Pool]:::app
    A --> S[(🚀 Cognito Identity Pool)]:::core
    S --> D[(📝 Temporary AWS Credentials)]:::data
    D --> V[📈 Access S3 / API / AWS Resources]:::dash

    classDef user fill:#F4B84A,stroke:#B86A00,stroke-width:4px,color:#111111,font-weight:bold;
    classDef app fill:#59C3E6,stroke:#2E5BFF,stroke-width:4px,color:#111111,font-weight:bold;
    classDef data fill:#B784F7,stroke:#6C2BD9,stroke-width:4px,color:#111111,font-weight:bold;
    classDef core fill:#FF2E93,stroke:#B1005D,stroke-width:4px,color:#FFFFFF,font-weight:bold;
    classDef dash fill:#7DDA8B,stroke:#2E9E44,stroke-width:4px,color:#111111,font-weight:bold;

    linkStyle 0,1,2,3 stroke:#EAEAEA,stroke-width:3px;
```
## Summary Table

| Topic | What It Is | How It Works | Best Use Case | Exam Trigger |
|---|---|---|---|---|
| Lambda | Serverless compute | Runs code on events, auto-scales | Event-driven apps | No servers, pay per use, auto scaling |
| Lambda Concurrency Error | Throttling from concurrency limits | Too many simultaneous invocations | Traffic spikes | Concurrent executions, throttled, quota reached |
| Lambda Cold Starts & Provisioned Concurrency | Cold start delay and warm capacity | Pre-initialized environments reduce startup delay | Low-latency APIs | Consistent latency, predictable traffic |
| Reserved and Provisioned Concurrency | Capacity control vs warm environments | Reserved protects capacity, provisioned reduces cold starts | Critical APIs needing capacity and speed | Guarantee capacity vs reduce startup delay |
| Lambda SnapStart | Snapshot-based cold start reduction | Restores from initialized snapshot | Slow-starting supported runtimes | Published version, cold start reduction |
| Lambda@Edge & CloudFront Function | Edge compute options for CloudFront | Functions for lightweight logic, Lambda@Edge for complex logic | Rewrites, headers, redirects, edge processing | Simple viewer logic vs complex edge logic |
| Amazon DynamoDB | Managed serverless NoSQL DB | Tables, items, attributes, auto scaling | High-scale low-latency apps | NoSQL, key-value, single-digit ms |
| Amazon DynamoDB Accelerator (DAX) | In-memory cache for DynamoDB | Serves cached DynamoDB reads | Hot read traffic | DynamoDB + microsecond read latency |
| DAX vs. ElastiCache | Purpose-built cache vs general cache | DAX for DynamoDB, ElastiCache for broader caching | DynamoDB read cache vs sessions/general cache | Minimal DynamoDB caching vs Redis/Memcached features |
| DynamoDB – Stream Processing | CDC from DynamoDB table changes | Stream records consumed by Lambda or others | Trigger actions on table changes | React to inserts, updates, deletes |
| DynamoDB – Stream Processing vs Kinesis Data Streams (Newer) | Simple CDC vs advanced streaming | Streams for simple use, Kinesis for replay/multi-consumer/retention | Event trigger vs analytics pipeline | 24h simple CDC vs long retention and replay |
| AWS API Gateway | Managed API front door | Routes requests to Lambda/HTTP/AWS services | Serverless APIs | Create, secure, throttle APIs |
| AWS API Gateway - Endpoint Types | Ways to expose REST APIs | Edge-Optimized, Regional, Private | Global public, regional public, internal VPC APIs | Global clients, same-region, VPC-only |
| AWS API Gateway - Security | API access control layers | IAM, Cognito, Lambda authorizer, resource policy, WAF | Secure public or private APIs | JWT, SigV4, custom auth, VPC endpoint restriction |
| AWS Step Functions | Workflow orchestration service | State machine with retries and branching | Multi-step business workflows | Orchestration, retries, long-running workflow |
| Amazon Cognito | Managed identity for apps | User Pools authenticate, Identity Pools give AWS creds | App login plus AWS resource access | Sign-in, JWT, temporary AWS credentials |