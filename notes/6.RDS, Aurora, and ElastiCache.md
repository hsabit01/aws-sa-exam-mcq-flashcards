...
### Use Case
A company runs an e-commerce app on EC2 and needs a managed MySQL database. They choose RDS so AWS handles backups, patching, and failover instead of the team building all of that manually. :contentReference[oaicite:2]{index=2}

### Exam Tip
Pick RDS when the question wants a **managed relational database** and the app still needs **SQL, joins, transactions, and normal database engines**. Good clue words are **managed database**, **automated backups**, **patching**, **Multi-AZ**, and **read replica**. A common trap is choosing EC2-hosted databases when the question clearly prefers less operational work. :contentReference[oaicite:3]{index=3}
...
### Use Case
If your app has too many read queries, add a **Read Replica**.  
If your app must survive an AZ failure with automatic failover, use **Multi-AZ**. :contentReference[oaicite:7]{index=7}

### Exam Tip
This is a favorite SAA comparison.

Clues for **Read Replica**:
- read-heavy workload
- reporting queries
- scale reads
- cross-Region replica
- eventual or slightly stale data is acceptable :contentReference[oaicite:8]{index=8}
...
- protect against AZ failure :contentReference[oaicite:9]{index=9}

Common trap:
- **Multi-AZ is not mainly for read scaling**
- **Read Replica is not mainly for HA failover**
- **Read Replica uses async replication**
- **Multi-AZ uses sync replication** :contentReference[oaicite:10]{index=10}
...
### Use Case
A fast-growing SaaS app needs a managed relational database with high performance, read scaling, and strong availability. Aurora is a strong fit when standard RDS is not enough. :contentReference[oaicite:13]{index=13}

### Exam Tip
Choose Aurora when the question wants:
...
- cloud-optimized database design :contentReference[oaicite:14]{index=14}

Common trap:
If the scenario only needs a simple managed relational DB, regular RDS may be enough and can be cheaper. Aurora is usually the better answer when the scenario emphasizes **performance**, **availability**, and **read scaling**. :contentReference[oaicite:15]{index=15}
...
### Use Case
A production database needs daily protection, point-in-time restore, and alarms when CPU or connections spike. Use automated backups, snapshots, CloudWatch alarms, and Performance Insights. :contentReference[oaicite:24]{index=24}

### Exam Tip
Watch for these clues:
- **recover to a specific time yesterday** → point-in-time recovery
- **keep a backup even after deleting DB** → manual snapshot
- **need OS-level visibility** → Enhanced Monitoring
- **need SQL wait / load analysis** → Performance Insights or Database Insights
- **need API audit trail** → CloudTrail :contentReference[oaicite:25]{index=25}

Common trap:
Do not confuse **backup/recovery** with **high availability**. Backups help restore data. Multi-AZ helps keep the database running after failure. :contentReference[oaicite:26]{index=26}
...
### Use Case
A company stores sensitive customer records in Aurora PostgreSQL. They place the cluster in private subnets, restrict access with security groups, enable KMS encryption, enforce TLS, and use Secrets Manager for credentials. :contentReference[oaicite:33]{index=33}

### Exam Tip
Strong clues:
- **encrypt database at rest** → KMS
- **encrypt client connection** → SSL/TLS
- **rotate or manage DB credentials** → Secrets Manager
- **restrict who can reach the DB** → security groups / private subnets
- **avoid hardcoded DB passwords** → IAM auth or Secrets Manager, depending on scenario :contentReference[oaicite:34]{index=34}

Common trap:
Do not confuse **IAM access to AWS APIs** with **logging in to the database engine itself**. API permissions and DB user authentication are different layers. :contentReference[oaicite:35]{index=35}
...
### Use Case
A serverless app using AWS Lambda opens too many database connections and overwhelms the database. Put RDS Proxy in front of the RDS or Aurora database to stabilize connection usage. :contentReference[oaicite:38]{index=38}

### Exam Tip
Big clue words:
- **too many DB connections**
- **Lambda + relational database**
- **connection pooling**
- **improve failover behavior**
- **make database connections more efficient** :contentReference[oaicite:39]{index=39}

Common trap:
RDS Proxy is **not a cache** and **not a read replica**. It helps with **connection management** and resilience, not read scaling. :contentReference[oaicite:40]{index=40}
...
### Use Case
A shopping site has product data that is read constantly. Instead of hitting the database every time, the app stores popular product data in ElastiCache to reduce latency and database load. :contentReference[oaicite:43]{index=43}

### Exam Tip
Choose ElastiCache when the question says:
- reduce database load
- microsecond or very low latency
- session store
- leaderboards / counters
- hot data
- repeated reads of the same data :contentReference[oaicite:44]{index=44}

Common trap:
ElastiCache improves speed, but it is **not the system of record**. The main durable data usually still lives in a database like RDS or DynamoDB. :contentReference[oaicite:45]{index=45}
...
### Use Case
A company stores user sessions in Redis. They keep the cache in private subnets, allow access only from the app tier security group, enable TLS, and use RBAC or IAM auth to limit access. :contentReference[oaicite:52]{index=52}

### Exam Tip
Strong clues:
- **need a secure Redis/Valkey cache in VPC** → subnet groups + security groups
- **encrypt cache traffic** → TLS
- **fine-grained Redis user access** → RBAC
- **AWS identity-based auth for Redis/Valkey** → IAM authentication :contentReference[oaicite:53]{index=53}

Common trap:
Do not treat ElastiCache like a public internet-facing service. Exam answers usually prefer **private subnets**, **restricted security groups**, and **app-tier-only access**. :contentReference[oaicite:54]{index=54}
