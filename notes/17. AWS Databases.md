...
| AWS Keyspaces | Managed serverless Apache Cassandra-compatible wide-column NoSQL database | Apps use CQL and Cassandra drivers while AWS handles scaling, storage, and HA | Migrating Cassandra workloads or running large-scale wide-column apps without managing clusters | Cassandra, CQL, wide-column, serverless, migrate existing Cassandra app |
| Amazon Timestream | Managed serverless time-series database | Writes recent data to memory store, moves older data to magnetic store, and supports analytics over both | IoT data, telemetry, DevOps metrics, monitoring dashboards | Time-stamped data, metrics, IoT sensors, trend analysis, recent vs historical data |